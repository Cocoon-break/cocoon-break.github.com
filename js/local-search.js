...
var searchFunc = function (path, search_id, content_id) {
    ...
    success: function (xmlResponse) {
      ...
      $input.addEventListener('input', function () {
        ...
        dataList.forEach(function (data) {
          ...
          var orig_data_content = data.content.trim().replace(
            /<[^>]+>/g, "");
            ...
            if (first_occur >= 0) {
              ...
              keywords.forEach(function (keyword) {
                ...
                match_content = match_content.replace(regS,
                  "<span class='pink-text'>" + keyword +
                  "</span>");
              });
              str += "<p class='search-list-content'>" +
                match_content + "...</p>"
            }
          }
        });
        if (str.indexOf('list-group-item') === -1) {
          return $('#local-search-input').addClass("invalid").removeClass(
            "valid");
        }
        $('#local-search-input').addClass("valid").removeClass(
          "invalid");
  ...
  $(document).on('click', '#local-search-close', function () {
    $('#local-search-input').val('').removeClass("invalid").removeClass(
      "valid");
